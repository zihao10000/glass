...
// 异常消息类型
#define MV_LGS_EXCEPTION_DEV_DISCONNECT          0x00008001      // 设备断开连接
#define MV_LGS_EXCEPTION_SOFTDOG_DISCONNECT      0x00008002      // 加密狗掉线
...
typedef struct _MVLGS_IMAGE_INFO_
{
	...
	unsigned char       chUserDefinedName[16];  // 用户自定义设备名称
    long long           nllTimeStamp;           // ch:数据接收到的时间戳
...
typedef struct _MVLGS_CODE_LIST_
{
    ...
    unsigned int        nRunNumber;                     // ch:流水号
...
typedef struct _MVLGS_VOLUME_INFO_
{
    ...
    unsigned int        nReserved[58];          // ch:保留 | en:Reserved
}MVLGS_VOLUME_INFO;
