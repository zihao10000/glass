...
************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_SetAreaInfoConfig(IN void* handle, IN unsigned int nAreaAddress, IN unsigned int nAreaCode);
 ...
 *  @param  void* handle                  [IN]        相机句柄
 *  @return 成功,返回最佳packetsize,失败,返回错误码
 
 *  @fn     MV_CODEREADER_GetOptimalPacketSize()
 *  @brief  Get the optimal Packet Size, Only support GigE Camera
 *  @param  void* handle                  [IN]        Camera handle
 *  @return Success, return Optimal packetsize. Failure, return error code
 ************************************************************************/
 ...
 *  @brief  从相机读取文件（虚拟相机不支持），通过UDP通信协议传输
 *  @param  handle                [IN]           句柄地址
 *  @param  pstFileAccess         [IN]           文件存取结构体
 *  @return 成功，返回MV_CODEREADER_OK；错误，返回错误码 
 
 *  @fn     MV_CODEREADER_FileAccessRead()
 *  @brief  Read the file from the camera (virtual camera does not support), transfer by UDP communication protocol
 *  @param  handle                [IN]           Handle
 ...
 *  @brief  将文件写入相机（虚拟相机不支持），通过UDP通信协议传输
 *  @param  handle                [IN]           句柄地址
 *  @param  pstFileAccess         [IN]           文件存取结构体
 *  @return 成功，返回MV_CODEREADER_OK；错误，返回错误码 
 
 *  @fn     MV_CODEREADER_FileAccessWrite()
 *  @brief  Write the file to camera (virtual camera does not support), transfer by UDP communication protocol
 *  @param  handle                [IN]           Handle
 *  @param  pstFileAccess         [IN]           File access structure
 *  @return Success, return MV_CODEREADER_OK. Failure, return error code
 ************************************************************************/
...
/************************************************************************
*  @fn     MV_CODEREADER_FileAccessReadByTCP()
*  @brief  从相机读取文件（虚拟相机不支持），通过TCP通信协议传输
*  @param  handle                [IN]           句柄地址
*  @param  pstFileAccess         [IN]           文件存取结构体
*  @return 成功，返回MV_CODEREADER_OK；错误，返回错误码

*  @fn     MV_CODEREADER_FileAccessReadByTCP()
*  @brief  Read the file from the camera (virtual camera does not support), transfer by TCP communication protocol
*  @param  handle                [IN]           Handle
*  @param  pstFileAccess         [IN]           File access structure
*  @return Success, return MV_CODEREADER_OK. Failure, return error code
************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_FileAccessReadByTCP(IN void* handle, IN MV_CODEREADER_FILE_ACCESS_EX * pstFileAccess);

/************************************************************************
*  @fn     MV_CODEREADER_FileAccessWriteByTCP()，通过TCP通信协议传输
*  @brief  将文件写入相机（虚拟相机不支持）
*  @param  handle                [IN]           句柄地址
*  @param  pstFileAccess         [IN]           文件存取结构体
*  @return 成功，返回MV_CODEREADER_OK；错误，返回错误码

*  @fn     MV_CODEREADER_FileAccessWriteByTCP()
*  @brief  Write the file to camera (virtual camera does not support), transfer by TCP communication protocol
*  @param  handle                [IN]           Handle
*  @param  pstFileAccess         [IN]           File access structure
*  @return Success, return MV_CODEREADER_OK. Failure, return error code
************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_FileAccessWriteByTCP(IN void* handle, IN MV_CODEREADER_FILE_ACCESS_EX * pstFileAccess);
...
/********************************************************************//**
 *  @~chinese
*   @fn     MV_CODEREADER_SetEnumDevTimeout
 *  @brief  设置枚举超时时间
 *  @param  nMilTimeout                 [IN]            超时时间，应为无符号整数,默认100ms
 *  @return 成功，返回MV_CODEREADER_OK；错误，返回错误码 
 *  @remarks 在调用MV_CODEREADER_EnumDevices等枚举接口前使用该接口，可设置枚举GIGE设备的网卡最大超时时间（默认100ms）,可以减少最大超时时间，来加快枚举GIGE设备的速度

 *  @~english
 *  @brief  Set enumerate device timeout
 *  @param  nMilTimeout                 [IN]            time out,input of unsigned int,default 100ms
 *  @return Success, return MV_CODEREADER_OK. Failure, return error code 
 *  @remarks Before calling enum device interfaces,call MV_CODEREADER_EnumDevices to set max timeout,can reduce the maximum timeout to speed up the enumeration of GigE devices.
 ************************************************************************/
MV_CODEREADERCTRL_API int __stdcall MV_CODEREADER_SetEnumDevTimeout(IN unsigned int nMilTimeout);
