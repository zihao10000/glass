...
// 文件存取
typedef struct _MV_CODEREADER_FILE_ACCESS_EX_
{
    char*               pUserFileBuf;                               // 用户输入缓存
    unsigned int        nFileBufSize;                               // 用户输入缓存大小
    unsigned int        nFileBufLen;                                // 文件数据缓存大小
    const char*         pDevFileName;                               // 设备文件名

    unsigned int        nReserved[32];                              // 预留
}MV_CODEREADER_FILE_ACCESS_EX;
...
typedef struct _MV_CODEREADER_RESULT_BCR_
{
    ...
    unsigned short          nNoReadNum;                                 // 有码无读数量
    unsigned short          nRes;                                       // 保留字节
    unsigned int            nReserved[3];                               // 保留字节
...
// 颜色信息
typedef struct _MV_CODEREADER_COLOR_
{
    unsigned int nRed;
    unsigned int nGreen;
    unsigned int nBlue;
...
// AGV坐标点信息
typedef struct _MV_CODEREADER_AGV_POINT_
{
    MV_CODEREADER_POINT_I      stPoint;
    MV_CODEREADER_COLOR        stColor;
    unsigned int               nRadius;    // 渲染点半径大小
}MV_CODEREADER_AGV_POINT;

// AGV渲染框信息
typedef struct _MV_CODEREADER_AGV_BOX_
{
    MV_CODEREADER_POINT_I      stPoint[4];
    MV_CODEREADER_COLOR        stColor;
    unsigned int               nEdgeWidth; // 边框线宽度
}MV_CODEREADER_AGV_BOX;

#define MAX_AGV_POINT_NUM  (32)
#define MAX_AGV_BOX_NUM    (20)
#define MAX_AGV_STRING_LEN (256)

// AGV读码头信息列表
typedef struct _MV_CODEREADER_AGV_INFO_
{
    unsigned int                    nPointNum;                                      // 渲染点信息数量
    MV_CODEREADER_AGV_POINT         stPoint[MAX_AGV_POINT_NUM];                     // 渲染点信息
    unsigned int                    nBoxNum;                                        // 渲染框信息数量
    MV_CODEREADER_AGV_BOX           stBoxInfo[MAX_AGV_BOX_NUM];                     // 渲染框信息
    unsigned int                    nStringLen;                                     // 字符实际真实长度
    unsigned char                   chDrawString[MAX_AGV_STRING_LEN];               // 渲染字符串信息
    unsigned int                    nReserved[8];                                   // 保留字节

}MV_CODEREADER_AGV_INFO;
...
typedef struct _MV_CODEREADER_IMAGE_OUT_INFO_EX_2_
{
    ...
    union
    {
        MV_CODEREADER_AGV_INFO*        pstAgvInfo;              // AGV读码头信息
        int64_t                        nAligning;
    }UnparsedAgvInfo;

    unsigned int                      nReserved[23];             // 保留字节
...
typedef enum _MV_CODEREADER_CODE_TYPE_
{
	...
	MV_CODEREADER_WITHCODE_NOREAD_SD = 1002,                // 有码无读堆叠码
