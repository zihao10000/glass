...
public:
    ...
    int     Open(MV_CODEREADER_DEVICE_INFO* pstDeviceInfo);
    ...
    int     GetOneFrameTimeout(unsigned char* pData, unsigned int* pnDataLen, unsigned int nDataSize, MV_CODEREADER_IMAGE_OUT_INFO_EX2* pFrameInfo, int nMsec);
    ...
    // ch:设置Command型参数
    // en:Set Command type parameters
    int     SetCommandValue(IN const char* strKey);
